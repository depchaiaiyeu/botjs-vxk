...
export async function processAndSendSticker(api, message, mediaSource, width, height) {
  ...
  try {
    ...
    if (!isLocalFile) {
      ...
      if (ext === "jxl") {
        mediaSource = mediaSource.replace(/\.jxl$/i, ".jpg")
        ext = "jpg"
      }
      ...
      if (ext === "jxl") {
        const newPath = mediaSource.replace(/\.jxl$/i, ".jpg")
        fs.renameSync(mediaSource, newPath)
        pathSticker = newPath
        ext = "jpg"
      } else {
    ...
    const finalUrl = linkUploadZalo[0].fileUrl + "?createdBy=Vu-Xuan-Kien-Service.BOT"
