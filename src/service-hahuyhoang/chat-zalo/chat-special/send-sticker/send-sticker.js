...
export async function processAndSendSticker(api, message, mediaSource) {
  ...
  try {
    ...
    if (ext === "jxl") {
      ...
      if (!convertSuccess) {
        throw new Error("Không thể chuyển định dạng .jxl sang .png sau khi thử tất cả phương pháp");
      }
    ...
    const finalUrl =
      linkUploadZalo[0].fileUrl + "?createdBy=Vu-Xuan-Kien-Service.BOT" ||
      linkUploadZalo[0].normalUrl + "?CreatedBy=HàHuyHoàng.BOT";
...
export async function handleStickerCommand(api, message) {
  ...
  try {
    ...
    if (!isAdminBot && mediaCheck.isVideo) {
      await sendMessageWarning(api, message, `${senderName}, Đại ca tao không cho phép thành viên tạo sticker video.`, true);
      return;
    }
